...
def tab3_bar_store(chosen_shop):
    ...
    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['Store_type']==chosen_shop)].groupby('Weekday')['total_amt'].sum()
    grouped = grouped.reindex(['Monday', 'Tuesday', 'Wednesday','Thursday', 'Friday', 'Saturday','Sunday'])
    fig = go.Figure(data=[go.Bar(x=grouped.index,y=grouped.values)],layout=go.Layout(title='Sprzedaż w poszczególne dni tygodnia'))
    return fig
...
def tab3_pie_country(chosen_shop):
    ...
    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['Store_type']==chosen_shop)].groupby('country')['total_amt'].sum()
    fig = go.Figure(data=[go.Pie(labels=grouped.index,values=grouped.values)],layout=go.Layout(title='Sprzedaż w poszczególnych krajach'))
    return fig
